...
// ERC20token contract can be deployed separately and its address can be user here instead of creating new instance of ERC20token.
    ...
    struct transaction {
		...
		address recipient;         //user who recieved the funds. 
		bytes32 secretHash;     
		bool burn_verified;
		bool mint_verified;    
   ...
   function get_mint_txn_details(uint _transactionId) public view returns (bool)
    {   
               return (transactions[_transactionId].mint_verified);
    }
   ...
   function burntverified(uint _transactionId ) public 
   {
     ...
     function Asset_mint(uint _transactionId, string memory _BCName, address _sender, uint _amount,  bytes32 _secretHash, bytes32 mintsecret) public 
    {   
