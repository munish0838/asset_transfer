...
sendFunds = async(amountFunded, receiverAddress,  secretHash, amountExpected, getTransactionID) => {
...
// if (this.state.chain === "Ethereum" || this.state.chain === "Binance") {
  ...
  // console.log(expectedAddress)
      ...
      const txn_details = await this.state.asset.methods.get_txn_details(getTransactionID).call();
      console.log(txn_details)

  

      const eAmount = txn_details.amounttransfered;
      console.log(eAmount)
      console.log(amountExpected)
      const eReceiver = txn_details.recipient;
      console.log(eReceiver)
      const bVerified = txn_details.burn_verified;
      console.log(bVerified)
      const eHash = txn_details.secretHash;
      console.log(eHash)
      console.log("Entering methiod now")
      console.log(eAmount===amountExpected)

      console.log(bVerified===true)
      console.log(eHash===secretHash)
      // console.log(eReceiver===this.state.account)
      console.log("If")
      if(eAmount==amountExpected && bVerified===true && eHash===secretHash ){

  // DO EDITS HERE FOR OUR SEND FUNCTION TO SEND TRANSACTION HASH TO OTHER CONTRACT/BLOCK CHAIN   }
  this.state.asset.methods.Asset_mint(this.state.chain, receiverAddress,amountExpected, secretHash ).send( { from: this.state.account, value: amountFunded},
    async(err,result) =>
  {
    if(err){
      console.log("Error in transactions")
    }
    // const txId = result.events.fundsburned.returnValues._currentTransactionId
    // console.log("current burn id is", txId)/
    ...
    // console.log(txId)
    console.log("end-2")
    console.log("Send function began here")
    console.log("Transaction Recepit below here")
    ...
    console.log("Send function at end")
    // const txId = txnRcp.transactionIndex
    // console.log(this.state.asset.methods.get_txn_details(txId))
    ...
    return(
        ...
        <div>
            ...
            <form onSubmit={(event) => {
                      ...
                      const getTransactionID = this.transactionID.value
                      ...
                      this.sendFunds(amountFunded, receiverAddress,  secretHash, amountFunded, getTransactionID)
                    }}>
                ...
                <div className="form-group my-4">
                    <div className="float-left">Address of  Sender</div>
                    <input className="form-control" id = "addressOfReceiver" type = "text" placeholder ="Type sender address here " ref={(input) => { this.addressOfReceiver = input }} required />
                </div>
                

                <div className="form-group my-4">
                    <div className="float-left">Transaction ID</div>
                    <input id="email" className="form-control" type="text" placeholder="Enter Transaction ID " ref={(input) => { this.transactionID = input }}  required />
                    {/*  */}
                </div>
