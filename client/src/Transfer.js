...
// function Transfer() {
    ...
    loadWeb3 = async() => {
        ...
        if (network === 'Ethereum') {
          console.log("ethereum load web3")
          if (window.ethereum) {
        ...
        // Binance wallet check assuming same functions as in ethereum
          ...
          console.log("Binance network dtetected")
	...
	// if (this.state.chain === "Ethereum" || this.state.chain === "Binance") {
      ...
      this.state.asset.methods.Asset_burn(this.state.chain, receiverAddress, secretHash ).send( { from: this.state.account, value: 0})
      .then( async(result) => 
